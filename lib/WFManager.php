...
class WFManager {
    ...
    static function open_session($course_id) {
        ...
        $data_course->startdate = mktime(0, 0, 0);
        $data_course->timemodified = time();
    ...
    static function discard_course($course_id, $course_category) {
        ...
        if (isCourseHubAvailable()) {
            ...
            if ($hub->getPublishedCourse($CFG->academie_name, $course_id, CourseHub::PUBLISH_PUBLISHED)) {
                $hub->unpublishCourse($course_id, CourseHub::PUBLISH_PUBLISHED);
            } else if($hub->getPublishedCourse($CFG->academie_name, $course_id, CourseHub::PUBLISH_SHARED)) {
                $hub->unpublishCourse($course_id, CourseHub::PUBLISH_SHARED);
            }
        }
