    ...
    public function processForm($data = null){
        ...
        if($data != null) {
            ...
            if($fpinfo['filecount'] != 0){
                ...
                //Cleanup old files:
                $fs->delete_area_files($data->contextid, 'user', 'workflowcsvenrol_'.$this->role);
                $fs->delete_area_files($usercontext->id, 'user', 'draft', $data->formateur_files_filemanager);
                ...
                $fs->create_file_from_string($fileinfo, $log);
            } else {

                $wf_files = $fs->get_area_files($data->contextid, 'user', 'workflowcsvenrol_'.$this->role, 0, '', false);
                $wf_files_array = array();
                foreach ($wf_files as $file) {
                    $wf_files_array[] = $file->get_filename();
                }

                $draft_files = $fs->get_area_files($usercontext->id, 'user', 'draft', $data->formateur_files_filemanager, '', false);
                $draft_files_array = array();
                foreach ($draft_files as $file) {
                    $draft_files_array[] = $file->get_filename();
                }

                foreach ($wf_files as $file) {
                    if (in_array($file->get_filename(), array_diff($wf_files_array, $draft_files_array))){
                        $file->delete();
                    }
                }
