...
function dialog_discard($courseid){
    ...
    $text = "";
    if ($published) {
        $text = '<tr><td>Ce parcours est partagé sur l’offre de parcours à déployer / publier pour inscription, la mise à la corbeille induira la suppression de cette publication</td></tr>';
    }
...
function dialog_publish($courseid, $method="share", $isalocalsession = 0){
    $random = rand(0, 999999999);
    $text = "Vous êtes sur le point de publier / mettre à jour ce parcours sur l'offre de parcours à déployer.";
    if($method == "publish"){
        if($isalocalsession){
            $text = "Vous êtes sur le point de publier / mettre à jour cette session sur l'offre de formation en libre accès locale.";
        } else {
            $text = "Vous êtes sur le point de publier / mettre à jour cette session sur l'offre de formation en libre accès.";
        }
    }
    $nexturl = new moodle_url("/local/workflow/ajax/WFApi.php");
...
function dialog_unpublish($courseid, $method="share"){
    $random = rand(0, 999999999);
    $text = "Vous êtes sur le point de retirer ce parcours de l'offre de parcours à déployer.";
    if($method == "publish"){
        $text = "Vous êtes sur le point de retirer ce parcours de l'offre de formation en libre accès.";
    }
    $nexturl = new moodle_url("/local/workflow/ajax/WFApi.php");
    
    return '
<div id="wf_dialog_un'.$method.'" style="display:none;">
    <form  method="POST" action="'.$nexturl.'" name="unpublish_form" id="wf_un'.$method.'_form">
