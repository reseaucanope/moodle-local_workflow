...
require_once($CFG->dirroot.'/local/workflow/pdfOptimizerReport.php');
    ...
    $params->module == 'convertunusedfilestocr' && is_int($params->courseid) && $params->courseid > 0 && strlen($params->session) == 25 && is_array($params->files) )
{
    ...
    foreach($filesSortedBySection AS $fileid){
        if (array_key_exists($fileid, $session->filestoconvert)) {
            // if (rand(1,50)<25){
            $resourceid = $optimizer->centralize_used_file_and_replace($fileid);
            if ($resourceid) {
                $session->filestoconvert[$fileid]->resourceid = $resourceid;
    ...
    $params->module == 'getresults' && is_int($params->courseid) && $params->courseid > 0 && strlen($params->session) == 25 )
{
    ...
    if ((isset($session->unusedfiles_succeed) && count($session->unusedfiles_succeed)) 
        || (isset($session->filestoconvert_succeed) && count($session->filestoconvert_succeed))) {
        $pdfPath = createPdfReport($params->courseid, get_mmid().CFO_SESSION_DATA);
        $resMail = sendMail($params->courseid, $session->startTime, $USER, $pdfPath);
        unlink($pdfPath);
    }
